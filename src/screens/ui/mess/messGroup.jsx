...
const handlePressOnMessage = (index, messageId) => {
  setSelectedMessageIndex(index); // Lưu chỉ số của tin nhắn được nhấn
  setSelectedMessageId(messageId); // Lưu ID của tin nhắn được nhấn
...
const handleDelete = (messageId) => {
  ...
  if (messageToEdit.content === "") {
    ...
    // Gửi yêu cầu cập nhật tin nhắn đến máy chủ
    recallMessagesGroups(group._id, dataUpdateMessage)
      .then(res => {
        if (res.data.response === "Bạn không phải là chủ tin nhắn") {
          alert("Bạn không phải là chủ tin nhắn nên không thể cập nhật");
        ...
        // Cập nhật trạng thái của tin nhắn thành đã thu hồi
        const updatedMessages = messagesGroups.map(message => {
        ...
        setMessagesGroups(updatedMessages);
...
{messagesGroups.map((message, index) => (
  <TouchableOpacity
    key={index}
    onPress={() => handlePressOnMessage(index, message._id)}
  >
    <View
      style={[
        styles.message,
        ...
        <View style={styles.messageTextContainer}>
          ...
          {showFunctionButtons && selectedMessageIndex === index && (
            <View style={styles.functionButtonsContainer}>
              ...
              <TouchableOpacity onPress={() => handleDelete(selectedMessageId)}>
...
const styles = StyleSheet.create({
  ...
  messageContainer: {
    flexDirection: "row",
    marginTop: 5,
    marginBottom: 10, // Thêm một khoảng cách từ phía dưới của tin nhắn
  },
  ...
  functionButtonsContainer: {
    ...
    alignItems: "center",
    position: "absolute",
    right: 0, // Đặt nút chức năng ở bên phải của tin nhắn
    paddingHorizontal: 20,
    paddingVertical: 10,
    marginBottom: 10, // Thêm một khoảng cách từ phía dưới của tin nhắn
    bottom: -40,
